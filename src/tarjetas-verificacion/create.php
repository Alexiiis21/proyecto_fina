...
$tarjeta = [
    ...
    'HoraSalida' => date('H:i', strtotime('+1 hour')),
    'MotivoVerificacion' => '',
    'FolioCertificado' => '',
    'Vigencia' => date('Y-m-d', strtotime('+6 months')),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($tarjeta['FolioCertificado'])) {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM tarjetasverificacion WHERE FolioCertificado = ?");
        $stmt->execute([$tarjeta['FolioCertificado']]);
        if ($stmt->fetchColumn() > 0) {
            $errores[] = "El folio de certificado ya existe. Por favor, use otro.";
        }
    }
    ...
    if (empty($errores)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO tarjetasverificacion (
                    ID_Vehiculo, ID_Centro_Verificacion, FechaExpedicion, 
                    HoraEntrada, HoraSalida, MotivoVerificacion, 
                    FolioCertificado, Vigencia, ID_Tarjeta_Circulacion, NumeroSerieVehiculo
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            ...
            if ($result) {
                $id = $pdo->lastInsertId();
                $_SESSION['success'] = "Tarjeta de verificación creada correctamente.";
                header("Location: view.php?id=$id");
                exit;
            } else {
                $_SESSION['error'] = "Error al crear la tarjeta de verificación.";
            }
        } catch (PDOException $e) {
...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>Nueva Tarjeta de Verificación</h1>
            ...
            <form method="POST" class="needs-validation" novalidate>
                ...
                <div class="row mt-4">
                    <div class="col-12 d-flex justify-content-end">
                        ...
                        <button type="submit" class="btn btn-primary">Guardar Tarjeta de Verificación</button>
                    </div>
                </div>
            </form>
        </div>
