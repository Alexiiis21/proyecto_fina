...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($pago['NumeroTransaccion'])) {
        try {
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM pagos WHERE NumeroTransaccion = ? AND ID_Pago != ?");
            $stmt->execute([$pago['NumeroTransaccion'], $id]);
            if ($stmt->fetchColumn() > 0) {
                $errores[] = "El número de transacción ya existe en otro pago. No se pueden duplicar transacciones.";
            }
        } catch (PDOException $e) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function verificarFechas() {
        if (fechaLimite.value && fechaPago.value && fechaPago.value > fechaLimite.value) {
            // Crear una advertencia visual
    ...
    // Verificar al cargar y cuando cambian las fechas
    verificarFechas();
    fechaPago.addEventListener('change', verificarFechas);
    fechaLimite.addEventListener('change', verificarFechas);
});
</script>
